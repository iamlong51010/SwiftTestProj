...
final class UserData {
    ...
    enum EPublishMsgType : String {
        case recordlistchange
        case languageset
    }
...
//    @Published var arrReqRecord : [RequestRecord] = []
    ...
    public let pubSubjectRecordList = PublishSubject<UserData.EPublishMsgType>()
    private(set) var localizedBundle : Bundle = Bundle.main
    private(set) var language : Int = 0 {
        didSet {
            let path = Bundle.main.path(forResource:self.language.toLanguageType.toLanguageStr , ofType: "lproj")
            self.localizedBundle = Bundle(path: path!) ?? Bundle.main
            self.pubSubjectRecordList.onNext(.languageset)
        }
    }
    private(set) var arrReqRecord : [RequestRecord] = [] {
        didSet {
            self.pubSubjectRecordList.onNext(.recordlistchange)
        }
    }
    ...
    */
    ...
    func loadUserData() -> Void {
        ...
        if FileOpe.CreateFileInDocument(subDirName: UserData.kSubdirName, fileName: UserData.kFileName) {
            self.arrReqRecord = FileOpe.LoadArrayInDocumentFile(subDirName: UserData.kSubdirName, fileName: UserData.kFileName) ?? []
        } else {
            self.arrReqRecord = []
        }
    ...
    private func resetReqRecordListID() {
        for index in 0..<self.arrReqRecord.count {
            self.arrReqRecord[index].id = UserData.kMinRecordId+index
        }
    }
    func addReqRecord(date:Date, content:RecordData) -> RequestRecord? {
        if self.arrReqRecord.count >= UserData.kMaxRecordIndex {
            self.arrReqRecord.removeFirst()
            let newRecId = self.arrReqRecord.last!.id+1
            self.arrReqRecord.append(RequestRecord(id: newRecId, date: date, content: content))
            resetReqRecordListID()
        } else {
            var newRecId = UserData.kMinRecordId
            if(self.arrReqRecord.count > 0) {
                newRecId = self.arrReqRecord.last!.id+1
            }
            self.arrReqRecord.append(RequestRecord(id: newRecId, date: date, content: content))
        }
        recordArrayChanged = true
        //refreshRelatedDataAfterRecListChanged()
        return self.arrReqRecord.last
    }
    func removeAllReqRecord() -> Void {
        if self.arrReqRecord.count > 0 {
            self.arrReqRecord.removeAll()
            recordArrayChanged = true
            //refreshRelatedDataAfterRecListChanged()
        }
    }
    func removeReqRecord(fromid id:Int) -> RequestRecord? {
        ...
        for index in 0..<self.arrReqRecord.count {
            if self.arrReqRecord[index].id == id {
                result = self.arrReqRecord.remove(at: index)
                recordArrayChanged = true
                break
            }
        }
...
//        }
    ...
    func removeReqRecord(fromindex index:Int) -> RequestRecord? {
        var result : RequestRecord? = nil
        if index >= 0 && index < self.arrReqRecord.count {
            result = self.arrReqRecord.remove(at: index)
            recordArrayChanged = true
        }
//        if result != nil {
...
//        }
    ...
    func writeCurReqRecordListInLocalFileIfNeed() -> Void {
        if recordArrayChanged {
            ...
            _ = FileOpe.WriteArrayInDocumentFile(list: self.arrReqRecord, subDirName: UserData.kSubdirName, fileName: UserData.kFileName)
        }
    }
